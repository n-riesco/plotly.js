...
module.exports = function draw(gd) {
    ...
    var legend = getLegend(),       // root element
        clipPath = getClipPath(),   // legend clip path
        bg = getBackground(),       // legend background
        scrollBox = getScrollBox(), // contains the labels for all the traces
        scrollBar = getScrollBar(), // scroll bar (visible only if needed)
        groups = getGroups(),       // contains the labels for all the traces
                                    // (for setting the gap between groups)
        traces = getTraces();       // the labels for all the traces
    ...
    function getTraces() {
        ...
        traces.call(style)
            ...
            .each(function(d, i) {
                drawTraceLabels(this, gd, d, i, traces);
    ...
    function drawTraceLabels(context, gd, d, i, traces) {
        var fullLayout = gd._fullLayout,
            trace = d[0].trace,
